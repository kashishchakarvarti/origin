 ...
 * Used by the order-volume algorithm to apply demand spikes.
 */

/** @typedef {"USA"|"Canada"|"UK"|"Australia"|"Germany"|"France"|"Japan"|"Singapore"|"UAE"|"India"} Country */

/** Shared retail / commerce peaks */
const GLOBAL = {
...
/** @type {Record<string, Record<string, { name: string; boost: number }>>} */
const BY_COUNTRY = {
  USA: {
 ...
 * @returns {{ name: string; boost: number } | null}
 */
export function getHoliday(date, country) {
  const key = `${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`;
  const local = BY_COUNTRY[country]?.[key];
